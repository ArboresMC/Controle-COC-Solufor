...
class ClosingListView(LoginRequiredMixin, ListView):
    ...
    template_name = 'compliance/closing_list.html'
    context_object_name = 'closings'

    def get_queryset(self):
        qs = MonthlyClosing.objects.select_related('participant', 'reviewed_by')
...
class SubmitCurrentClosingView(LoginRequiredMixin, FormView):
    template_name = 'compliance/submit_closing.html'
    form_class = MonthlyClosingForm

    def get_closing(self):
        today = date.today()
        return MonthlyClosing.objects.get_or_create(
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['closing'] = self.get_closing()
        return ctx

    def form_valid(self, form):
        closing = self.get_closing()
        if closing.status == 'approved':
            messages.error(self.request, 'Este fechamento já foi aprovado e não pode ser reenviado.')
            return redirect('closing_list')
        closing.participant_notes = form.cleaned_data['participant_notes']
        closing.declaration_no_movement = form.cleaned_data['declaration_no_movement']
        closing.status = 'submitted'
        closing.submitted_at = timezone.now()
        closing.save()
        messages.success(self.request, 'Fechamento enviado com sucesso.')
        return redirect('closing_list')

class ManagerClosingListView(ManagerRequiredMixin, ListView):
    model = MonthlyClosing
    template_name = 'compliance/manager_closing_list.html'
    context_object_name = 'closings'
...
class ManagerClosingApproveView(ManagerRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        closing = get_object_or_404(MonthlyClosing, pk=kwargs['pk'])
        ...
        return redirect('manager_closing_list')
...
class ManagerClosingRejectView(ManagerRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        closing.reviewed_by = request.user
        closing.save()
        messages.success(request, 'Fechamento rejeitado.')
        return redirect('manager_closing_list')
