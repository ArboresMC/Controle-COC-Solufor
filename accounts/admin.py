...
class UserAdmin(BaseUserAdmin):
    fieldsets = BaseUserAdmin.fieldsets + (('Perfil no portal', {'fields': ('role', 'organization', 'participant', 'must_change_password')}),)
    ...
    def save_model(self, request, obj, form, change):
        if not request.user.is_superuser:
            if obj.participant_id and obj.participant and obj.participant.organization_id: obj.organization = obj.participant.organization
            elif not obj.organization_id: obj.organization = request.user.current_organization
