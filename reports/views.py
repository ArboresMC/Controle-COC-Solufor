...
class TraceabilityReportView(ManagerRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        rows_paginator = Paginator(rows, 50)
        rows_page = rows_paginator.get_page(request.GET.get('page'))

        entry_balances = get_entry_balance_rows(participant=participant)
        balances_paginator = Paginator(entry_balances, 50)
        balances_page = balances_paginator.get_page(request.GET.get('balances_page'))
        ...
        return render(request, self.template_name, {
            ...
            'entry_balances_page_obj': balances_page,
        })
