    ...
    if 'Transformacoes' in workbook.sheetnames:
        ...
        for idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
            ...
            try:
                ...
                preview = {
                    'linha': idx, 'data': data, 'source_product': source_product.name, 'source_quantity': source_quantity,
                ...
                if persist:
                    obj = TransformationRecord.objects.create(
                        participant=participant, movement_date=_normalize_date(data), source_product=source_product,
