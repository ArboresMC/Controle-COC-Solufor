...
from transactions.services import build_traceability_rows, convert_to_base, calculate_target_from_source, get_transformation_rule
...
class ConsolidatedExcelReportView(ManagerRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        ws4.append(['Participante', 'Produto', 'Tipo de uso', 'Uso', 'Data uso', 'Origem consumida', 'Data origem', 'Quantidade', 'Unidade'])
        for row in build_traceability_rows():
            ws4.append([str(row['participant']), str(row['product']), row['use_type'], row['use_label'], row['use_date'].strftime('%d/%m/%Y'), row['source_label'], row['source_date'].strftime('%d/%m/%Y'), float(row['quantity']), row['unit']])
...
class TraceabilityReportView(ManagerRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, self.template_name, {'rows': rows, 'participants': Participant.objects.filter(status='active'), 'selected_participant': participant})
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'Saidas' in workbook.sheetnames:
            ...
            for idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
                ...
                quantity_base = convert_to_base(product, quantidade, unidade)
                SaleRecord.objects.create(participant=participant, movement_date=data, document_number=str(documento), customer=customer, product=product, quantity=quantidade, movement_unit=unidade, unit_snapshot=product.unit, quantity_base=quantity_base, fsc_claim=declaracao or '', batch_code=lote or '', notes=observacoes or '', created_by=request.user, status='submitted')
                created['sales'] += 1
        ...
        if 'Transformacoes' in workbook.sheetnames:
            ...
            for idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
                ...
                TransformationRecord.objects.create(participant=participant, movement_date=data, source_product=source_product, source_quantity=quantidade_origem, source_unit=unidade_origem, source_quantity_base=source_quantity_base, target_product=target_product, target_quantity_base=target_quantity_base, target_unit_snapshot=target_product.unit, yield_factor_snapshot=rule.yield_factor, notes=observacoes or '', created_by=request.user)
