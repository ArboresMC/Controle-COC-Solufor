...
class TransformationRecord(models.Model):
    ...
    customer = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='transformations_as_customer',
    ...
    supplier = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='transformations_as_source_supplier',
        verbose_name='Fornecedor de origem',
    )
    fsc_claim = models.CharField('Declaração FSC', max_length=100, blank=True)
    ...
    def __str__(self):
        label = f'Transformação {self.source_product} → {self.target_product}'
        if self.document_number:
            label = f'{label} · {self.document_number}'
        return label
...
class TraceLot(models.Model):
    ...
    def label(self):
        ...
        if self.source_type == 'transformation' and self.transformation_id:
            doc = f' · {self.transformation.document_number}' if self.transformation.document_number else ''
            return f'Transformação #{self.transformation_id} → {self.product}{doc}'
