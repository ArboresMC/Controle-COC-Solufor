...
class BaseMovement(models.Model):
    ...
    def clean(self):
        ...
        if self.product and self.product.fsc_applicable and not (self.fsc_claim or '').strip():
            raise ValidationError({'fsc_claim': 'Selecione a declaração FSC para este produto.'})
...
class SaleRecord(BaseMovement):
    ...
    def clean(self):
        super().clean()
        if self.product and self.product.fsc_applicable and not (self.fsc_claim or '').strip():
            raise ValidationError({'fsc_claim': 'A saída precisa ter uma declaração FSC compatível com o lote de origem.'})
...
class TraceLot(models.Model):
    ...
    @property
    def label(self):
        claim = f' · {self.fsc_claim}' if self.fsc_claim else ''
        if self.source_type == 'entry' and self.entry_id:
            supplier = f' · {self.supplier}' if self.supplier_id else ''
            return f'Entrada {self.entry.document_number}{supplier}{claim}'
        if self.source_type == 'transformation' and self.transformation_id:
            return f'Transformação #{self.transformation_id} → {self.product}{claim}'
        return f'Lote #{self.pk}{claim}'
...
class LotAllocation(models.Model):
    ...
    def clean(self):
        ...
        if self.sale_id and self.sale.fsc_claim and self.lot.fsc_claim != self.sale.fsc_claim:
            raise ValidationError('A saída não pode consumir lote com declaração FSC diferente da informada.')
