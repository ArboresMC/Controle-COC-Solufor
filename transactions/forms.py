...
def _claim_name_from_field_value(value):
    if not value:
        return ''
    if isinstance(value, FSCClaim):
        return value.name
    try:
        return FSCClaim.objects.get(pk=value).name
    except (FSCClaim.DoesNotExist, ValueError, TypeError):
...
class BaseMovementForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        claim_value = getattr(self.instance, 'fsc_claim', '') if self.instance else ''
        if claim_value:
            claim_obj = FSCClaim.objects.filter(name=claim_value).first() or FSCClaim.objects.filter(code=claim_value).first()
            if claim_obj:
                self.fields['fsc_claim'].initial = claim_obj
    ...
    def clean(self):
        ...
        claim = cleaned.get('fsc_claim')
        cleaned['fsc_claim_name'] = _claim_name_from_field_value(claim)
...
class SaleRecordForm(BaseMovementForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        selected_claim = None
        if self.is_bound:
            selected_claim = _claim_name_from_field_value(self.data.get('fsc_claim')) or None
        ...
        if self.is_bound:
            ...
            if source_lot_id:
                try:
                    ...
                    if source_lot.fsc_claim:
                        claim_obj = FSCClaim.objects.filter(name=source_lot.fsc_claim).first()
                        if claim_obj:
                            self.fields['fsc_claim'].initial = claim_obj
    ...
    def clean(self):
        ...
        product = cleaned.get('product')
        fsc_claim = cleaned.get('fsc_claim_name', '')
        quantity = cleaned.get('quantity')
        movement_unit = cleaned.get('movement_unit')
        ...
        if source_lot and fsc_claim and (source_lot.fsc_claim or '') != fsc_claim:
