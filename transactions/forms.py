...
class BaseMovementForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['new_product_name'].help_text = (
            'Se informado, cria o produto usando a unidade selecionada como unidade base.'
        )
...
class EntryRecordForm(BaseMovementForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['supplier'].help_text = (
            'Opcional. Se deixar em branco, o sistema salvará como "Não informado".'
        )
...
class SaleRecordForm(BaseMovementForm):
    ...
    source_lot = forms.ModelChoiceField(
        label='Alocação manual da origem (opcional)',
        queryset=TraceLot.objects.none(),
        required=False,
        empty_label='Usar FIFO automático'
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['source_lot'].help_text = (
            'Opcional: selecione um lote específico para vincular a venda a uma compra ou '
            'transformação determinada. Em branco, o sistema usa FIFO automático.'
        )
        ...
        if participant:
            ...
            self.fields['source_lot'].queryset = (
                TraceLot.objects.filter(pk__in=lot_ids)
                .select_related('product', 'supplier', 'entry', 'transformation')
    ...
    def clean(self):
        ...
        if source_lot and quantity and movement_unit and product:
            try:
                ...
                available = get_lot_remaining_for_sale(
                    source_lot,
                    sale=self.instance if getattr(self.instance, 'pk', None) else None
                )
                if qty_base > available:
                    self.add_error(
                        'source_lot',
                        f'Lote sem saldo suficiente. Disponível: {available} {product.get_unit_display()}.'
                    )
            except Exception as exc:
...
class TransformationRecordForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if new_source and not source_product and source_unit:
            cleaned['new_source_product_payload'] = {
                'name': new_source,
                'unit': source_unit,
            }
        ...
        if source_product and source_quantity and source_unit and target_product:
            try:
                ...
                target_quantity_base = calculate_target_from_source(
                    source_product,
                    target_product,
