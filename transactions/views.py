...
class EntryCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        obj.supplier = form.cleaned_data.get('supplier') or obj.supplier
        obj.fsc_claim = form.cleaned_data.get('fsc_claim', '')
...
class SaleCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        obj.unit_snapshot = obj.product.unit
        obj.quantity_base = convert_to_base(obj.product, obj.quantity, obj.movement_unit)
        available = get_available_balance(self.request.user.participant, obj.product, fsc_claim=obj.fsc_claim)
...
class SaleUpdateView(SaleCreateView, UpdateView):
    ...
    def form_valid(self, form):
        ...
        available = get_available_balance(self.request.user.participant, obj.product, fsc_claim=obj.fsc_claim) + current_base
        if obj.quantity_base > available:
            form.add_error('quantity', f'Saldo insuficiente. Disponível: {available} {obj.product.get_unit_display()}. Solicitado: {obj.quantity_base} {obj.product.get_unit_display()}.')
            return self.form_invalid(form)
        preferred_lot = form.cleaned_data.get('source_lot')
