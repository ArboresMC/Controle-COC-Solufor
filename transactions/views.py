...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_manager:
            ...
            ctx['dashboard_alerts'] = get_manager_alerts(today=today)
        else:
...
class EntryCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        obj.fsc_claim = form.cleaned_data.get('fsc_claim_name') or ''
        obj.supplier = form.cleaned_data.get('supplier')
...
class SaleCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if obj.quantity_base > available:
            ...
            return self.form_invalid(form)
        preferred_lot = form.cleaned_data.get('source_lot')
        obj.supplier = None
        obj.fsc_claim = ''
        try:
            with transaction.atomic():
...
class TransformationCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        obj.source_quantity = form.cleaned_data['estimated_source_quantity']
        ...
        if obj.source_quantity_base > available:
            form.add_error('target_quantity', f'Saldo insuficiente do produto origem. Disponível: {available} {obj.source_product.get_unit_display()}. Necessário para esta transformação: {obj.source_quantity_base} {obj.source_product.get_unit_display()}.')
            return self.form_invalid(form)
        try:
            with transaction.atomic():
                obj.save()
