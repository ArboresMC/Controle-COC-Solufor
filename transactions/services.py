...
def get_transformation_claim(transformation):
    claims = sorted({(allocation.lot.fsc_claim or '').strip() for allocation in transformation.source_lot_allocations.select_related('lot') if (allocation.lot.fsc_claim or '').strip()})
    if not claims:
        return ''
    if len(claims) > 1:
...
def sync_transformation_target_lot(transformation):
    claim = get_transformation_claim(transformation)
    lot, _ = TraceLot.objects.get_or_create(
...
def _available_lots(participant, product, fsc_claim=None, *, sale=None, include_lot_ids=None):
    ...
    if include_lot_ids:
        include_qs = TraceLot.objects.filter(pk__in=include_lot_ids, participant=participant, product=product)
        if fsc_claim is not None:
            include_qs = include_qs.filter(fsc_claim=fsc_claim)
        lots = lots | include_qs
        lots = lots.order_by('movement_date', 'id')
    items = []
    seen = set()
    for lot in lots:
...
def get_manual_sale_lot_choices(participant, product=None, sale=None, fsc_claim=None):
    ...
    if fsc_claim is not None:
        qs = qs.filter(models.Q(fsc_claim=fsc_claim) | models.Q(pk__in=current_ids))
    if product:
...
def allocate_quantity_to_lots(participant, product, quantity_base, *, sale=None, transformation=None, preferred_lot=None, fsc_claim=None):
    ...
    if preferred_lot is not None:
        ...
        if fsc_claim is not None and preferred_lot.fsc_claim != fsc_claim:
            raise ValueError('O lote selecionado possui declaração FSC diferente da solicitada.')
    ...
    for lot, remaining in _available_lots(participant, product, fsc_claim=fsc_claim, sale=sale):
        if qty_needed <= 0:
            break
...
def get_available_balance(participant, product, statuses=None, fsc_claim=None):
    lots = TraceLot.objects.filter(participant=participant, product=product)
    if fsc_claim is not None:
        lots = lots.filter(fsc_claim=fsc_claim)
    total = Decimal('0')
...
def build_traceability_rows(participant=None, product=None):
    ...
    for allocation in allocations:
        ...
        if allocation.sale_id:
            ...
            use_type = 'transformação'
            use_label = f'Transformação #{allocation.transformation_id}'
            counterparty = 'Processo interno'
        ...
        rows.append({
            ...
            'fsc_claim': allocation.lot.fsc_claim,
        })
    return rows
