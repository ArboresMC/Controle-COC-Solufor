...
def sync_transformation_target_lot(transformation):
    ...
    source_allocations = transformation.source_lot_allocations.select_related('lot').all()
    ...
    lot.supplier = source_allocations[0].lot.supplier if source_allocations and source_allocations[0].lot.supplier_id else None
    lot.fsc_claim = source_claims[0] if len(source_claims) == 1 else ''
