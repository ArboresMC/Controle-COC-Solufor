...
def _available_lots(participant, product, *, sale=None, include_lot_ids=None):
    lots = TraceLot.objects.filter(participant=participant, product=product).order_by('movement_date', 'id')
    if include_lot_ids:
...
def get_manual_sale_lot_choices(participant, product=None, sale=None):
    current_ids = []
    if sale and getattr(sale, 'pk', None):
...
def allocate_quantity_to_lots(participant, product, quantity_base, *, sale=None, transformation=None, preferred_lot=None):
    ...
    for lot, remaining in _available_lots(participant, product, sale=sale):
        if qty_needed <= 0:
            break
...
def reallocate_sale(sale, preferred_lot=None):
    ...
    return allocate_quantity_to_lots(sale.participant, sale.product, sale.quantity_base, sale=sale, preferred_lot=preferred_lot)
