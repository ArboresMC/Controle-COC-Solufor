...
def convert_from_base(product, quantity_base, to_unit: str) -> Decimal:
    quantity_base = to_decimal(quantity_base)
    factor = get_unit_conversion_factor(product, to_unit)
    if factor == 0:
        raise ValueError('Fator de conversão inválido para a unidade informada.')
    return (quantity_base / factor).quantize(Decimal('0.001'))
