...
def get_unit_conversion_factor(product, from_unit: str) -> Decimal:
    ...
    if from_unit == base_unit:
        return Decimal('1')
    conversion = ProductUnitConversion.objects.filter(product=product, from_unit=from_unit, to_unit=base_unit, active=True).first()
    if conversion:
...
def get_transformation_rule(source_product, target_product, participant=None):
    qs = ProductTransformationRule.objects.filter(source_product=source_product, target_product=target_product, active=True)
...
def get_lot_remaining_for_sale(lot, sale=None):
    ...
    if sale and getattr(sale, 'pk', None):
        current = to_decimal(lot.allocations.filter(target_type='sale', sale=sale).aggregate(total=Sum('quantity_base'))['total'])
        remaining += current
...
def get_lot_remaining_for_transformation(lot, transformation=None):
    ...
    if transformation and getattr(transformation, 'pk', None):
        current = to_decimal(lot.allocations.filter(target_type='transformation', transformation=transformation).aggregate(total=Sum('quantity_base'))['total'])
        remaining += current
    return remaining.quantize(Decimal('0.001'))
...
def sync_entry_lot(entry):
    lot, _ = TraceLot.objects.get_or_create(entry=entry, defaults={'participant': entry.participant,'product': entry.product,'source_type': 'entry','supplier': entry.supplier,'fsc_claim': entry.fsc_claim,'movement_date': entry.movement_date,'quantity_base': entry.quantity_base,'unit_snapshot': entry.unit_snapshot})
...
def sync_transformation_target_lot(transformation):
    lot, _ = TraceLot.objects.get_or_create(transformation=transformation, defaults={'participant': transformation.participant,'product': transformation.target_product,'source_type': 'transformation','movement_date': transformation.movement_date,'quantity_base': transformation.target_quantity_base,'unit_snapshot': transformation.target_unit_snapshot})
    allocated = _lot_allocated_total(lot)
    if allocated > to_decimal(transformation.target_quantity_base):
        raise ValueError(f'A transformação #{transformation.id} já possui {allocated} {transformation.target_product.get_unit_display()} consumidos em saídas. Não é possível reduzir a produção abaixo desse valor.')
...
def _available_lots(participant, product, *, sale=None, include_lot_ids=None, fsc_claim=None, supplier=None):
    ...
    items, seen = [], set()
    for lot in lots:
        if lot.id in seen: continue
        seen.add(lot.id)
        remaining = get_lot_remaining_for_sale(lot, sale=sale) if sale else get_lot_remaining(lot)
        if remaining > 0: items.append((lot, remaining))
    return items

def get_manual_sale_lot_choices(participant, product=None, sale=None, fsc_claim=None):
    current_ids = list(sale.lot_allocations.values_list('lot_id', flat=True)) if sale and getattr(sale, 'pk', None) else []
    qs = TraceLot.objects.filter(participant=participant)
    if fsc_claim: qs = qs.filter(fsc_claim=fsc_claim)
    if product: qs = qs.filter(models.Q(product=product) | models.Q(pk__in=current_ids))
    elif current_ids: qs = qs.filter(models.Q(pk__in=current_ids) | models.Q())
    ...
    for lot in qs:
        ...
        lots.append({'lot': lot,'remaining': remaining.quantize(Decimal('0.001')),'supplier': supplier_label,'source_label': source_label,'unit': lot.product.get_unit_display()})
    return lots

def get_manual_transformation_lot_choices(participant, transformation=None, product=None):
    current_ids = list(transformation.source_lot_allocations.values_list('lot_id', flat=True)) if transformation and getattr(transformation, 'pk', None) else []
...
def allocate_quantity_to_lots(participant, product, quantity_base, *, sale=None, transformation=None, preferred_lot=None, fsc_claim=None):
    ...
    allocations = []; selected_supplier = None; selected_claim = _normalize_claim(fsc_claim)
    if preferred_lot is not None:
        ...
        if remaining < qty_needed:
            raise ValueError(f'Lote selecionado sem saldo suficiente. Disponível neste lote: {remaining.quantize(Decimal("0.001"))} {product.get_unit_display()}.')
        selected_supplier = preferred_lot.supplier; selected_claim = _normalize_claim(preferred_lot.fsc_claim)
        allocation = LotAllocation.objects.create(participant=participant, lot=preferred_lot, sale=sale, transformation=transformation, target_type='sale' if sale else 'transformation', quantity_base=qty_needed.quantize(Decimal('0.001')))
        if sale is not None:
            sale.supplier = selected_supplier; sale.fsc_claim = selected_claim; sale.save(update_fields=['supplier', 'fsc_claim'])
        return [allocation]
    for lot, remaining in _available_lots(participant, product, sale=sale, fsc_claim=selected_claim or None):
        ...
        if selected_claim and lot_claim != selected_claim: continue
        if not selected_claim: selected_claim = lot_claim
        if selected_supplier is None: selected_supplier = lot.supplier
        elif lot.supplier_id != getattr(selected_supplier, 'id', None): raise ValueError('A saída não pode consumir lotes de fornecedores diferentes.')
        if selected_claim and lot_claim != selected_claim: raise ValueError('A saída não pode consumir lotes com declarações FSC diferentes.')
        consumed = min(remaining, qty_needed)
        allocation = LotAllocation.objects.create(participant=participant, lot=lot, sale=sale, transformation=transformation, target_type='sale' if sale else 'transformation', quantity_base=consumed.quantize(Decimal('0.001')))
        allocations.append(allocation); qty_needed -= consumed
    if sale is not None:
        sale.supplier = selected_supplier; sale.fsc_claim = selected_claim or ''; sale.save(update_fields=['supplier', 'fsc_claim'])
    if qty_needed > 0:
        target_desc = f'saída {sale.document_number}' if sale else f'transformação #{transformation.id if transformation and transformation.id else "nova"}'
        raise ValueError(f'Não existe saldo rastreável suficiente para {target_desc}. Faltam {qty_needed.quantize(Decimal("0.001"))} {product.get_unit_display()}.')
    return allocations

@transaction.atomic
def reallocate_sale(sale, preferred_lot=None):
    sale.lot_allocations.all().delete(); sale.supplier = None; sale.save(update_fields=['supplier'])
    return allocate_quantity_to_lots(sale.participant, sale.product, sale.quantity_base, sale=sale, preferred_lot=preferred_lot, fsc_claim=sale.fsc_claim)
...
def classify_balance(value):
    ...
    if value <= Decimal('1'): return 'danger'
    if value <= Decimal('5'): return 'warning'
    return 'success'
...
def _origin_entry_ids_for_lot(lot, visited=None):
    ...
    visited.add(key)
    if lot.entry_id: return {lot.entry_id}
...
def get_manager_alerts(*, today=None, organization=None):
    ...
    if organization:
        corrections_entries = corrections_entries.filter(participant__organization=organization)
        corrections_sales = corrections_sales.filter(participant__organization=organization)
    else:
