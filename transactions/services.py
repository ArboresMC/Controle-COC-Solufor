...
def _available_lots(participant, product):
    lots = TraceLot.objects.filter(participant=participant, product=product).order_by('movement_date', 'id')
    return [(lot, get_lot_remaining(lot)) for lot in lots if get_lot_remaining(lot) > 0]



def allocate_quantity_to_lots(participant, product, quantity_base, *, sale=None, transformation=None):
    qty_needed = to_decimal(quantity_base)
    if qty_needed <= 0:
        return []
    allocations = []
    for lot, remaining in _available_lots(participant, product):
        if qty_needed <= 0:
            break
...
@transaction.atomic
def reallocate_sale(sale):
    sale.lot_allocations.all().delete()
    return allocate_quantity_to_lots(sale.participant, sale.product, sale.quantity_base, sale=sale)
